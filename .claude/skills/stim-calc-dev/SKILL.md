...
`scWeekGlanceLoadFromFirebase()` (real-time `.on('value')` listeners on d3Roadmap/monthlyPlanner + clinicalData + deadlines + upcomingDeadlines), `scWeekGlanceGetCurrentWeek()` (Mon-Sun date range), `scWeekGlanceRender()` (DOM-based, 2 sections: weekly schedule + 20-day deadlines capped at 15, max-height 500px scrollable), `scWeekGlanceFormatTime()`, `scWeekGlanceGetDeadlines(start, end)` (merges editedDeadlines + customDeadlines + upcomingDeadlines, excludes completed, deduplicates). Schedule items dedup by clinicalAppointmentId and date+time+name to prevent stacking.
