...
`logNicotine()`, `updateNicotineDisplay()`, `checkRLSRisk()`, `toggleModifier()`, `toggleAllNighterMode()`, `renderGhostLoad()`, `initWorkoutPlanner()`, `calculateWorkoutImpact()`, `applyWorkoutPlan()`, `updateScenarios()`, `updateForecastLogic()`, `clearToday()` (clears inputs without archiving), `resetDay()` (archives prediction to history via saveDay()+saveDailyLogicLog(), then clears meds/caffeine/modifiers/workout/nicotine/allNighter — preserves wakeTime and hoursSlept)
...
`scWeekGlanceLoadFromFirebase()` (real-time `.on('value')` listeners on d3Roadmap: currentWeekSchedule, customTasks, appointments, deadlines, upcomingDeadlines), `scWeekGlanceGetCurrentWeek()` (Mon-Sun date range), `scWeekGlanceRender()` (DOM-based, 2 sections: weekly schedule + 20-day deadlines capped at 15 with Xd badges, card flex 800px scrollable), `scWeekGlanceFormatTime()`, `scWeekGlanceGetDeadlines(start, end)` (merges editedDeadlines + customDeadlines + upcomingDeadlines, excludes completed, deduplicates). **Primary schedule source**: `currentWeekSchedule` (pre-built by d3-roadmap initMonthlyPlanner, includes static tasks + custom + clinical, pre-deduped). Fallback: customTasks + appointments (missing static tasks like lectures/labs).
