...
- **`applyRequirementCheckoffs` (V2)**: COMPLETED_TODAY (`isDelta: true`) creates procedure records on patient but does NOT touch competency `item.completed` counts. Passes `competencyItemIds: []` to recordProcedure. REQUIREMENTS_STATUS (`!isDelta`) sets absolute `item.completed` counts AND sets `item.lastVerified` to today's local date. Only REQUIREMENTS_STATUS modifies competency counts.
...
- **Competencies UI (V2)**: Atlas Console design system with `cv2-*` CSS classes. Light theme (#f7f5ef canvas). 3 panels: milestone KPI strip (sticky), D3 deadline alert + category accordion, What's Next. Mobile-first with 44px touch targets.
...
- **Pipeline data stays on patient records**: `importedRequirements[]` on patient records (from CAN_FULFILL) is the source for pipeline badges. NOT stored on competency items. `getPatientsFulfilling()` queries patient records.
